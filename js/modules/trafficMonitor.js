...
let urlRegionId = '', urlAni = '', urlDuration = '', CMECodeList = [], backUrl = '', filterAni = '', selected_log_id = '';
let loadNextLength = 20, loadPreviousLength = 20;
...
const openMessageLogsModal = (logs) => {
    const obj = JSON.parse(base64Decode(logs));
    selected_log_id = obj['id'];
    logsTables.filtersMessageLogs.searchList[0]['value'] = obj;
...
const logsTables = {
    filtersMessageLogs: {
        searchList: [
            {"field": "data", "operator": "is", "value": ''},
            {"field": "loadNextLength", "operator": "is", "value": loadNextLength},
            {"field": "loadPreviousLength", "operator": "is", "value": loadPreviousLength}
        ]
    },
    columnsMessageLogs: getColumnName(),
    columnDefsMessageLogs: getColumnDef('sub'),
    orderMessageLogs: [],
    messageLogsTable: function () {
        callCaseAlertDataTable('call_box_log_table', 'liveMessage/tableListEventLogById', logsTables.filtersMessageLogs, logsTables.columnsMessageLogs, function (settings) {
            var response = settings.json;
            if(response['nextTotal'] < loadNextLength) {
                $('.load-more-next').addClass('d-none');
            } else {
                $('.load-more-next').removeClass('d-none');
            }
            if(response['previousTotal'] < loadPreviousLength) {
                $('.load-more-previous').addClass('d-none');
            } else {
                $('.load-more-previous').removeClass('d-none');
            }
            $('#messageLogsModal').modal('show');
        }, logsTables.columnDefsMessageLogs, logsTables.orderMessageLogs, function (row, data) {
            if (data.id === selected_log_id) {
                $(row).css("background-color", '#bbbb00');
                $('#messageLogRegionName').html('<tbody>' + $(row).html() + '</tbody>');
            }
        });
    },
};
...
function loadMore(type = 'next') {
    if(type === 'next') {
        loadNextLength = loadNextLength + 20;
    } else {
        loadPreviousLength = loadPreviousLength + 20;
    }
    logsTables.filtersMessageLogs.searchList[1]['value'] = loadNextLength;
    logsTables.filtersMessageLogs.searchList[2]['value'] = loadPreviousLength;
...
function getColumnDef(table_type = 'main') {
    return [
        {
            "targets": [0],
            render: function (ani_number, type, full) {
                if (ani_number != null) {
                    if (table_type === 'main') {
                        let ani_html = '<span style="cursor: pointer;color: #0b7ec4;" onclick="openMessageLogsModal(\'' + base64Encode(JSON.stringify(full)) + '\')">';
                        ...
                        return full.manually_set_start_time ? ani_number + '<i class="ml-1 fa fa-info-circle text-info" title="Call start time is estimated, and not actual"></i>' : ani_number;
                    }
                } else {
                    return '';
                }
